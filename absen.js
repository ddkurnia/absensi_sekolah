...
class AttendanceController {
  ...
  async init() {
    try {
      ...
      // 2. Ambil konfigurasi sheet dari user data (sudah termasuk sheetId, folderId, dll)
      if (this.currentUser.sheetId) {
        this.sheetConfig = {
          sheetId: this.currentUser.sheetId,
          folderId: this.currentUser.folderId,
          schoolName: this.currentUser.schoolName,
          spreadsheetUrl: this.currentUser.spreadsheetUrl
        };
      } else {
        // Fallback: cek storage lama
      ...
      // 6. Load system config dari Firestore (WhatsApp settings dari Master Admin)
      if (typeof SystemConfigService !== 'undefined' && SystemConfigService.loadSystemConfig) {
        await SystemConfigService.loadSystemConfig();
      }

      // 7. Inisialisasi Google API jika Google Drive sudah terhubung
      if (this.sheetConfig && typeof GoogleDriveService !== 'undefined') {
        try {
          await GoogleDriveService.initGoogleAPI();
        } catch (e) {
          console.warn('[Absen] Google API init failed:', e);
        }
      }

      // 8. Buat instance SheetsService
      if (typeof SheetsService === 'function' && this.sheetConfig && this.sheetConfig.sheetId) {
        this.sheetsService = new SheetsService(this.sheetConfig.sheetId);
      }

      // 9. Buat instance WhatsAppService jika diaktifkan
      if (typeof WhatsAppService === 'function' && this.whatsappEnabled) {
        this.whatsAppService = new WhatsAppService();
      }

      // 10. Setup listener online/offline
      this.setupNetworkListeners();

      // 11. Update indikator koneksi
      this._updateNetworkIndicator();

      // 12. Load daftar kelas
      await this.loadClasses();

      // 13. Cek dan proses antrian offline
      await this._checkOfflineQueue();

      // 14. Mulai auto-save timer
