...
// Load GAS_URL dari pengaturan Google Drive
function loadGasUrl() {
    try {
...
function showPage(id) {
    ...
    if(id === 'settings-page') {
        // Sembunyikan tab Google Drive untuk non-admin
        const gdriveTab = document.getElementById('tabBtnGdrive');
        if(gdriveTab) {
            if(currentUserRole === 'admin') {
                gdriveTab.style.display = '';
            } else {
                gdriveTab.style.display = 'none';
                // Jika sedang di tab gdrive, pindah ke tab jam
                const activeTab = document.querySelector('.tab-btn.active');
                if(activeTab && activeTab.dataset.tab === 'gdrive') {
                    switchSettingsTab('jam');
                }
            }
        }
        loadAllSettings();
...
// ===== 8. INITIALIZATION =====
function initSystem() {
    loadGasUrl();  // Load GAS URL dari pengaturan
    updateOnlineStatus();
    loadProfile();
    loadSettings();
    updateDashboardUI();
...
function loadAllSettings() {
    ...
    loadProfileSettings();
    renderJadwalList();
    loadGoogleDriveStatus();  // Load Google Drive connection status
}
...
function loadGoogleDriveStatus() {
    ...
    const statusDesc = document.getElementById('gdriveStatusDesc');
    ...
    if (config && config.connected) {
        ...
        if (statusText) {
            ...
            statusText.className = 'text-sm font-semibold text-slate-600';
        }
        if (statusDesc) {
            statusDesc.textContent = 'Hubungkan Google Drive untuk mengaktifkan sinkronisasi otomatis';
        }
